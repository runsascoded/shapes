...
export function update_log_level(level) {
    wasm.update_log_level(addHeapObject(level));
}
...
export function make_model(inputs, targets) {
    const ret = wasm.make_model(addHeapObject(inputs), addHeapObject(targets));
    return takeObject(ret);
}
...
export function xyrr_unit(xyrr) {
    const ret = wasm.xyrr_unit(addHeapObject(xyrr));
    return takeObject(ret);
}
...
async function __wbg_init(input) {
    ...
    if (typeof input === 'undefined') {
        input = new URL('apvd_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
